...
# 权重文件自动下载函数
def download_weight_file(url, filename):
    if os.path.exists(filename):
        return
    if "github.com" in url and not url.endswith(".pt"):
...
# Streamlit缓存加载模型，自动从GitHub下载权重文件
@st.cache_resource
def load_model():
    weight_url = "https://github.com/BUCMGYY/cervical-instability-classifier/raw/main/best_xt50.pt"
    weight_file = "best_xt50.pt"
    download_weight_file(weight_url, weight_file)
